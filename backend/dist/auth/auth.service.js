...
let AuthService = class AuthService {
    ...
    createJwtAccessToken(email, inviteCode) {
        return __awaiter(this, void 0, void 0, function* () {
            const jwtToken = jsonwebtoken_1.sign({ email: email, inviteCode: inviteCode }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRATION });
            return this.encryptText(jwtToken);
        });
    }
    hashCreatorData(token) {
        return __awaiter(this, void 0, void 0, function* () {
            const hashed = this.exampleJwtExtractor(token);
            return hashed;
        });
    }
    ...
    exampleJwtExtractor(request) {
        try {
            let token = request;
            const cryptr = new Cryptr(process.env.ENCRYPT_JWT_SECRET);
            token = cryptr.decrypt(token);
            const result = jsonwebtoken_1.verify(token, process.env.JWT_SECRET);
            return result;
        }
        catch (err) {
            return 'error';
